...
class DataExplorer:
    # jezeli <= 10 uniq var to uznaje ze kol kategoryczna 
    def __init__(self, df: pd.DataFrame, cat_threshold: int = 10):
        self.df = df.copy()   # kopia dataframe zeby nie nadpisac danych
        self.cat_threshold = cat_threshold 
        self._detect_column_types()

    def _detect_column_types(self):
        # funckja zwraca listy kolumn o danych typach
        # pandas sam wykrywa typ kolumny numeryczna opisowa kategoryczna
    ...
    def basic_stats(self, columns: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
        # jezeli nie podamy kolumny to bierze wszytskie numeryczne 
        if columns is None:
        ...
        result = {}
        for col in columns:
            series = self.df[col].dropna() # usuwa puste 
            if len(series) == 0:
            ...
            stats = {
                ...
                "skewness": float(series.skew()), # skosnosc rozklad przesuniety lewo / prawo
                "kurtosis": float(series.kurtosis()), # czy djest duzo odstajacych 
                "q25": float(series.quantile(0.25)), # kwartyle 
    ...
    def pca(self, columns: Optional[List[str]] = None, n_components: Optional[int] = None, standardize: bool = True) -> Dict[str, Any]:
        ...
        data = self.df[columns].dropna() # usuwa puste 
        if data.shape[0] < 2:
        ...
        if standardize:
            ...
            data_scaled = data.values # zmienia na numpy array

        n_comp = n_components or min(data.shape[0], data.shape[1])
        pca = PCA(n_components=n_comp) # analiza skladowych glownych
        pca.fit(data_scaled)
